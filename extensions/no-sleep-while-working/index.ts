  ...
  function startCaffeinate() {
    ...
    ac.signal.onabort = () => {
      ...
      try {
        ...
        console.error(
          "[no-sleep-while-working] Failed to kill caffeinate process:",
          error,
        );
      }
    };
    ...
    try {
      ...
      proc.on("error", (error) => {
        ...
        console.error(
          "[no-sleep-while-working] Caffeinate process error:",
          error,
        );
        ac.abort();
      });
      ...
      proc.on("exit", (code) => {
        if (code)
          console.error(
            `[no-sleep-while-working] Caffeinate process exited with non-zero status ${code}`,
          );
        ac.abort();
      });
    } catch (error) {
      console.error(
        "[no-sleep-while-working] Failed to start caffeinate:",
        error,
      );
      ac.abort();
    }
